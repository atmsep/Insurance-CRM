...
export default async function CashRegisterPage({
  ...
  let collectionsQuery = supabase
    ...
    .select(
      "id, installment_id, amount, paid_at, receipt_number, paid_by, " +
  ...
  const [{ data: rawCollections }, { data: rawReversals }, { data: agents }] = await Promise.all([
    collectionsQuery,
    reversalsQuery,
    isAdmin
              ...
              collections.map((c) => {
                ...
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <Link href={`/dashboard/policies/${one(c.policy_installments)?.policy_id}`} className="hover:underline">
                        {number}
                      </Link>
                    </TableCell>
